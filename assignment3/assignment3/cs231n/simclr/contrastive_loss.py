...
def sim(z_i, z_j):
    ...
    ##############################################################################
    product = torch.dot(z_i, z_j)
    norm_dot_product =  product / (torch.linalg.norm(z_i) * torch.linalg.norm(z_j))
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        l_k_k_N = -torch.log(numerator / denominator_k)
        ...
        l_k_N_k = -torch.log(numerator / denominator_k_N)
...
def sim_positive_pairs(out_left, out_right):
    ...
    norm_out_left = torch.linalg.norm(out_left, dim=1)
    norm_out_right = torch.linalg.norm(out_right, dim=1)
    denominator = torch.mul(norm_out_left, norm_out_right)
    pos_pairs = (numerator / denominator).reshape(-1, 1)
...
def compute_sim_matrix(out):
    ...
    # TODO: Start of your code.                                                  #
    ##############################################################################
    numerator = torch.matmul(out, out.T)
    norm_out = torch.linalg.norm(out, dim=1).reshape(-1, 1)
    denominator = torch.matmul(norm_out, norm_out.T)
